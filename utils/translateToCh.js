...
var getRegion = function(p, c) {
  ...
  var chinese_p, chinese_c;
  for (var i = 0; i < province.length; i++) {
    if (lower_p == province[i].name) {
      chinese_p = province2[i].name;
      for (var j = 0; j < province[i].cities.length; j++) {
        if (lower_c == province[i].cities[j]) {
          chinese_c =  province2[i].cities[j];
          return chinese_p + ' ' + chinese_c;
        }
      }
    }
  }
