...
const conf = {
  ...
  calculatePrevMonthGrids(year, month) {
    ...
    if (firstDayOfWeek > 0) {
      ...
      setData({
        'calendar.prev_disabled_day': empytGrids.reverse()
      });
    } else {
      setData({
        'calendar.prev_disabled_day': null
      });
    }
  },
  /**
  ...
  calculateNextMonthGrids(year, month) {
    ...
    if (+lastDayWeek !== 6) {
      ...
      setData({
        'calendar.last_disabled_day': lastEmptyGrids
      });
    } else {
      setData({
        'calendar.last_disabled_day': null
      });
    }
  },
  /**
  ...
  selectedDayWeekAllDays(currentDay) {
    ...
    setData({
      'calendar.days': days,
      'calendar.prev_disabled_day': empytGrids,
      'calendar.last_disabled_day': lastEmptyGrids
    });
  },
  /**
  ...
  switchWeek(view) {
    ...
    if (view === 'week') {
      ...
      if (currentPage.weekMode) return;  //作用？ 点击第一次无效 为了什么？
      currentPage.weekMode = true;   //page 可以直接申明赋值未存在变量
      setData({
        'calendar.week_mode': true
      });
      //console.log(currentPage);
...
export default (config = {}) => {
  ...
  setData({
    ...
    'calendar.week_mode': false  //修改
  });
  currentPage.weekMode = false; //修改
